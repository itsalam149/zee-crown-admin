    ...
    return (
        ...
        >
            ...
            <div className="space-y-2">
                ...
                <input id="title" name="title" type="text" defaultValue={notification?.title || ''} className="w-full bg-black/30 border-2 border-gray-700 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>

            <div className="space-y-2">
                <label htmlFor="message" className="block text-sm font-semibold text-gray-300">Message</label>
                <textarea id="message" name="message" rows={5} defaultValue={notification?.message || ''} className="w-full bg-black/30 border-2 border-gray-700 text-white rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-emerald-500" required />
            </div>
            ...
            <div className="flex justify-end pt-4">
                <button type="submit" disabled={isSubmitting} className="px-8 py-3 font-bold text-white bg-green-600 rounded-lg hover:bg-green-700 disabled:bg-gray-600">
                    {isSubmitting ? (notification?.id ? 'Updating...' : 'Creating...') : (notification?.id ? 'Update Notification' : 'Create Notification')}
                </button>
            </div>
        </form>
    );
