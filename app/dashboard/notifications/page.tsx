...
export default async function NotificationsPage() {
    ...
    if (error) {
        return <div className="p-8"><p className="text-red-400">Error loading notifications: {error.message}</p></div>;
    }
        ...
        <div className="min-h-screen bg-black text-gray-300 font-sans p-6 md:p-12">
            <div className="max-w-screen-2xl mx-auto">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
                    ...
                    <Link href="/dashboard/notifications/new" className="bg-green-600 hover:bg-green-700 text-white font-bold py-4 px-7 rounded-lg flex items-center space-x-3 text-lg shadow-lg shadow-green-600/20">
                        <Plus size={26} />
                    ...
                    <div className="overflow-x-auto">
                        <table className="min-w-full">
                            <thead className="bg-black/50 border-b border-green-500/20">
                                <tr>
                                    <th className="px-10 py-6 text-left text-base font-semibold text-gray-400 uppercase tracking-wider">Title</th>
                                    <th className="px-10 py-6 text-left text-base font-semibold text-gray-400 uppercase tracking-wider">Message</th>
                                    <th className="px-10 py-6 text-left text-base font-semibold text-gray-400 uppercase tracking-wider">Created At</th>
                            ...
                            <tbody className="divide-y divide-gray-800">
                                {notifications?.map((notification) => (
                                    <tr key={notification.id} className="hover:bg-green-950/20 transition-colors">
                                        <td className="px-10 py-8 text-xl font-medium text-white align-top">{notification.title}</td>
                                        <td className="px-10 py-8 text-lg text-gray-400 max-w-lg align-top">{notification.message}</td>
                                        <td className="px-10 py-8 text-lg text-gray-300 align-top">{new Date(notification.created_at).toLocaleString()}</td>
                                        <td className="px-10 py-8 whitespace-nowrap align-top">
                                            <div className="flex items-center space-x-6">
                                                <Link href={`/dashboard/notifications/${notification.id}/edit`} className="text-gray-400 hover:text-green-500 transition-colors"><Edit size={28} /></Link>
