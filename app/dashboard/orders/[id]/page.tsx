    ...
    return (
        <div className="min-h-screen bg-black text-gray-300 font-sans p-6 md:p-12">
            <div className="max-w-screen-xl mx-auto">
                <div className="mb-8">
                    <Link href="/dashboard/orders" className="inline-flex items-center space-x-2 text-green-400 hover:text-green-300 transition-colors">
                        <ArrowLeft size={20} />
                                ...
                                {orderItems?.map(item => (
                                    <li key={item.id} className="p-6 flex items-center space-x-6">
                                        {item.products?.image_url ? (
                                            <Image src={item.products.image_url} alt={item.products.name || 'Product Image'} width={64} height={64} className="w-16 h-16 rounded-lg object-cover border-2 border-gray-700" />
                                        ) : (
                                        ...
                                        <div className="flex-1">
                                            <p className="text-lg font-semibold text-white">{item.products?.name || 'Product not found'}</p>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="backdrop-blur-xl bg-black/30 p-6 rounded-2xl border border-green-500/20 space-y-4">
                            ...
                            <div>
                                ...
                                {/* @ts-ignore */}
                                <p className="text-lg font-semibold text-white">{order.profiles?.full_name || 'N/A'}</p>
                            </div>
                            <div>
                                <p className="text-sm text-gray-400">Date</p>
                                <p className="text-lg font-semibold text-white">{new Date(order.created_at).toLocaleString()}</p>
                            </div>
                            <div>
                            ...
                            <div className="pt-4 border-t border-gray-800">
                                ...
                                <span className={`px-3 py-1 text-sm font-semibold rounded-full capitalize ${getStatusStyles(order.status)}`}>
                                    {order.status}
                                </span>
                            </div>
                            <div className="pt-4 border-t border-gray-800 flex justify-between items-center">
                                ...
                                <p className="text-2xl font-bold text-green-400">
                                    {/* Use 0 as a fallback if total_price is null */}
